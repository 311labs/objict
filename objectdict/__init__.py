...
class ObjectDict(dict):
    ...
    parameter. If such a method is defined, then a call to `my_ObjectDict[key]`
    (or the equivalent `my_ObjectDict.__getitem__(key)`) that fails will call
    ...
    def __init__(self, *args, **kwargs):
        """
        Initialize a new `ObjectDict` using `dict.__init__`.
        ...
        the `ObjectDict`. For a recursive init that will convert all
        dict values in a dict to ObjectDicts, use `ObjectDict.fromdict`.
        Likewise, dotted keys will not be treated specially, so something
        like `ObjectDict({'a.b': 'a.b'})` is equivalent to `ud = ObjectDict()` followed
    ...
    def __reduce__(self):
        # pickle the contents of a ObjectDict as a list of items;
    ...
    def changes(self, dict2, ignore_keys=None):
        ...
        for k in self:
            ...
            if isinstance(v1, dict):
                if not isinstance(v1, ObjectDict):
                    v1 = ObjectDict.fromdict(v1)
                v = v1.changes(v2)
                if len(v):
                    changes[k] = v
            else:
    ...
    def fromKeys(self, keys):
        # generates a new ObjectDict, but only with the
        # passed in keys
        d = ObjectDict()
        for k in keys:
    ...
    def fromkeys(self, seq, value=None):
        return ObjectDict((elem, value) for elem in seq)
    ...
    def fromdict(cls, mapping, safe_keys=False):
        """
        Create a new `ObjectDict` from the given `mapping` dict.
        The resulting `ObjectDict` will be equivalent to the input
        `mapping` dict but with all dict instances (recursively)
        converted to an `ObjectDict` instance.  If you don't want
        this behavior (i.e., you want sub-dicts to remain plain dicts),
        use `ObjectDict(mapping)` instead.
        """
        ud = cls()
        for k in mapping:
            nk = k
    ...
    def todict(self, fields=None):
        """
        Create a plain `dict` from this `ObjectDict`.
        The resulting `dict` will be equivalent to this `ObjectDict`
        but with every `ObjectDict` value (recursively) converted to
    ...
    def copy(self):
        """
        Return a shallow copy of this `ObjectDict`.
        For a deep copy, use `ObjectDict.fromdict` (as long as there aren't
        plain dict values that you don't want converted to `ObjectDict`).
        """
        return ObjectDict.fromdict(self)
    ...
    def __dir__(self):
        ...
        are stored.
        """
        return sorted(set(dir(ObjectDict)) | set(self.keys()))
